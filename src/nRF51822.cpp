...
#define ATTRIBUTE_TABLE_SIZE		BLE_GATTS_ATTR_TAB_SIZE_MIN * 2
	...
	unsigned char numRemoteAttributes)
{
	...
	memset(&enableParams, 0x00, sizeof(ble_enable_params_t));
	...
	enableParams.gap_enable_params.central_conn_count = 1;
	enableParams.gap_enable_params.central_sec_count = 0;

	sd_ble_enable(&enableParams, &app_ram_base);

	ble_opt_t periph_ble_cfg_opt;
	ble_opt_t central_ble_cfg_opt;

	memset(&periph_ble_cfg_opt, 0x00, sizeof(ble_opt_t));
	...
	sble_enable_params_t enableParams = {
		.gatts_enable_params = {
			.service_changed = true
		}
	};
...
void nRF51822::poll(uint32_t* evtBuf, uint16_t* evtLen) {
...
#endif
			if (bleEvt->evt.gap_evt.params.connected.role != BLE_GAP_ROLE_PERIPH) {// Use this only for peripheral Connected event
				break;
			}
...
uint32_t nRF51822::startAdvertising() {
#ifdef NRF_51822_DEBUG
	...
	return sd_ble_gap_adv_start(&advertisingParameters);
}

uint32_t nRF51822::stopAdvertise()
{
